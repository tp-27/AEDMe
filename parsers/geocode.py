...
class Geocode:
    ...
    def store_results(self, action):

        if (action):
            #store valid results
            latLng_file = "../info_json/" + university + "/" + university + "_latlng.json"
            with open(latLng_file, 'w') as f:
                f.write(json.dumps(self.valid_results, indent=2))

            #store invalid results
            invalid_file = "../info_json/" + university + "/" + university + "_invalid.json"
            with open(invalid_file, 'w') as f:
                f.write(json.dumps(self.invalid_results, indent=2))
        
        else:  #append to current valid file
            #store valid results
            latLng_file = "../info_json/" + university + "/" + university + "_latlng.json"
            with open(latLng_file, 'a') as f:
                f.write(json.dumps(self.valid_results, indent=2))
    ...
    def run(self, action):  
        ...
        if action: #1 = valid addreses
            info_file = "../info_json/" + university + "/" + university + "_info.json"
        else:
            info_file = "../info_json/" + university + "/" + university + "_invalid.json"
...
if __name__ == '__main__':
    action = input("Press 1 to process valid addresses, 0 for invalid: ")
