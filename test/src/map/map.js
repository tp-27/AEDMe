...
function getAEDInfo(campus, currPosMarker) {
    const fileNames = {
        "guelph": "guelph_latlng.json",
        "waterloo": "waterloo_latlng.json",
        "carleton": "carleton_latlng.json",
        "ottawa": "ottawa_latlng.json",
        "toronto": "toronto_latlng.json",
        "mcmaster": "mcmaster_latlng.json",
        "manitoba": "manitoba_latlng.json",
        "brock": "brock_latlng.json"
    }
...
function createPanel(panel, nearestAEDS, currMarker) {
    ...
    for (let i = 0; i < nearestAEDS.length; i++) {
        ...
        infoDiv.onmouseover = function () { // add hover effect on info
            infoDiv.style.cssText = `
        ...
        infoDiv.addEventListener("click", (e) => {
            ...
            directionsService.route({
                origin: getLatLng(currMarker), // FIX ME - CURRMARKER WILLL BE A MARKER_INFO OBJECT AS WELL
...
function haversine_distance(marker1, currLocation) {
    ...
    let rlat2 = currLocation.position.lat() * (Math.PI/180); // Convert degrees to radians
    let difflat = rlat2-rlat1; // Radian difference (latitudes)
    let difflon = (currLocation.position.lng()-marker1.position.lng()) * (Math.PI/180); // Radian difference (longitudes)
